...
BackRefDescription = namedtuple(
    "BackRefDescription", "source destination name primaryjoin force_uselist_false"
)
...
class Relationship(object):
    ...
    def add_backref(self, name_backref=None, force_uselist_false=False):
        """
        ...
        if self.kwargs.get("uselist", False) or force_uselist_false:
            if GLOBAL_FLAGS.get("passive_deletes", False):
                backref_text = (
                    'backref("{}", uselist=False, passive_deletes=True)'.format(
                        name_backref
                    )
                )
            else:
...
class ManyToOneRelationship(Relationship):
    def __init__(
        self, source_cls, target_cls, constraint, inflect_engine, tables_with_backrefs
    ):
        ...
        if (source_cls, target_cls) in tables_with_backrefs:
            ...
            descriptions = tables_with_backrefs[(source_cls, target_cls)]
            backref_description = None

            add_backref = False
            if len(descriptions) == 1:
                backref_description = next(iter(descriptions))
                add_backref = True
            else:
                # we have multiple options, we need to retrieve the one for which the primaryjoin is the same
                primary_join = None
                if "primaryjoin" in self.kwargs:
                    primary_join = self.kwargs["primaryjoin"].replace("'", "")
                if primary_join:
                    for description in descriptions:
                        if description.primaryjoin == primary_join:
                            backref_description = description

            # we still need to ckech that the primary matches. Redundant if we had multiple elements in descriptions, but still required if there is one
            if backref_description and backref_description.primaryjoin:
                add_backref = False
                if (
                    "primaryjoin" in self.kwargs
                    and self.kwargs["primaryjoin"].replace("'", "")
                    == backref_description.primaryjoin
                ):
                    add_backref = True
            if add_backref:
                self.add_backref(backref_description.name, backref_description.force_uselist_false)
...
class ManyToManyRelationship(Relationship):
    def __init__(self, source_cls_obj, target_cls, assocation_table):
        ...
        if (
            len(foreign_keys_ass_to_source) == 1
            and foreign_keys_ass_to_source[0] in unique_columns_in_association_table
        ):
            self.kwargs["uselist"] = "False"
            self.preferred_name = (
                tablename if not colname.endswith("_id") else colname[:-3]
            )
...
{models}"""
                ...
                for constraint in table.constraints.copy():
                    if isinstance(constraint, CheckConstraint):
                        ...
                        try:
                            ...
                            logger.error(
                                "Error compiling contraint {}. Ignoring".format(e)
                            )
                            continue
    ...
    def render_column_type(cls, coltype):
        ...
        if isinstance(coltype, Enum):
            ...
            defaults = dict(
                zip(
                    argspec.args[-len(argspec.defaults or ()) :], argspec.defaults or ()
                )
            )
            missing = object()
            use_kwargs = False
    ...
    def render_class(self, model):
        ...
        # Render extra code for class
        snippet = self.extras.get(model.name, None)
        if snippet:
            rendered += "\n"
            rendered += textwrap.indent(snippet, self.indentation)
            rendered += "\n"
